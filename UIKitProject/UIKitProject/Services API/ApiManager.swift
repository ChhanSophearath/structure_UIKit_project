...
class ApiManagerAsyncAwait {
    ...
    private func createRequest(
        ...
        // Add body if present
        if let body = body {
            request.httpBody = try encoder.encode(body)/// Encodable: convert swift object to data
        } else if let parameters = parameters {
            request.httpBody = try JSONSerialization.data(withJSONObject: parameters)/// JSONSerialization: convert json to data
        }
